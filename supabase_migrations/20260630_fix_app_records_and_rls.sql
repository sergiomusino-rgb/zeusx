-- ============================================================================
-- ZeusX - Fix app_records + RLS completo (UNICO SCRIPT)
-- Data: 2026-06-30
-- Compatibilità: Supabase (PostgreSQL 15+)
-- Descrizione: Ricrea app_records con tutte le colonne, aggiunge colonne
--              mancanti alle altre tabelle, e crea TUTTE le policy RLS.
--              IDEMPOTENTE: può essere eseguito più volte senza errori.
--              DA COPIARE E INCOLLARE NEL SQL EDITOR DI SUPABASE.
-- ============================================================================

-- ============================================================================
-- FASE 0: Ricrea app_records con la struttura corretta
--         (DROP + CREATE per garantire che tutte le colonne esistano)
-- ============================================================================

-- Salva i dati esistenti in una tabella temporanea (se ci sono)
CREATE TEMP TABLE IF NOT EXISTS _app_records_backup AS
SELECT * FROM app_records WHERE 1=0; -- solo struttura, nessun dato (sicurezza)

-- Verifica se ci sono dati da preservare
DO $$
DECLARE
  record_count INT;
BEGIN
  SELECT COUNT(*) INTO record_count FROM app_records;
  IF record_count > 0 THEN
    -- Backup dei dati esistenti
    DROP TABLE IF EXISTS _app_records_backup;
    CREATE TEMP TABLE _app_records_backup AS SELECT * FROM app_records;
    RAISE NOTICE 'Backup di % record da app_records', record_count;
  END IF;
END $$;

-- Droppa la tabella esistente (qualsiasi struttura abbia)
DROP TABLE IF EXISTS app_records CASCADE;

-- Ricrea con TUTTE le colonne necessarie
...
-- Ripristina i dati dal backup (solo le colonne che esistevano)
DO $$
DECLARE
  restored_count INT;
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = '_app_records_backup' AND table_schema = 'pg_temp') THEN
    -- Inserisci solo le colonne comuni
    INSERT INTO app_records (id, app_id, tenant_id, table_name, data, created_at, updated_at)
    SELECT 
      id,
      COALESCE(app_id, (SELECT id FROM apps WHERE tenant_id = b.tenant_id LIMIT 1)),
      tenant_id,
      table_name,
      data,
      created_at,
      COALESCE(updated_at, created_at)
    FROM _app_records_backup b
    WHERE EXISTS (SELECT 1 FROM apps WHERE id = b.app_id OR tenant_id = b.tenant_id);
    
    GET DIAGNOSTICS restored_count = ROW_COUNT;
    RAISE NOTICE 'Ripristinati % record in app_records', restored_count;
    
    DROP TABLE IF EXISTS _app_records_backup;
  END IF;
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Backup ripristino saltato: %', SQLERRM;
END $$;
...
-- ============================================================================
-- FASE 6: Policy RLS (DROP + CREATE per tutte)
-- ============================================================================
